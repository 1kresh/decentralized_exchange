...
export default function Swap() {
  ...
  useEffect(() => {
    ...
    if (!!element && !!address) {
      clearDiv(element);
      if (isBlockIcon) {
        localStorage.setItem("isBlockIcon", "1");
        setBlockIcon(element, address);
      } else {
        localStorage.removeItem("isBlockIcon");
        setJazzIcon(element, address);
      }
    }
  }, [address, avatarRef, isBlockIcon, ethBalance, chainId]);
  ...
  useEffect(() => {
    ...
    const timeOutId = setTimeout(
      () => handleSearchTokenInput(chooseModalInput),
      275
    );
  ...
  useEffect(() => {
    if (
      (token0Amount == undefined || token0Amount == 0) &&
      tokenInputFocus == 0
    ) {
      setTokenInputFocus();
      setToken1Amount();
  ...
  useEffect(() => {
    if (
      (token1Amount == undefined || token1Amount == 0) &&
      tokenInputFocus == 1
    ) {
      setTokenInputFocus();
      setToken0Amount();
  ...
  const togglePage = (event) => {
    ...
    if (!!event.which) {
      // if event.which, use 2 for middle button
      if (event.which === 2) {
        openTab("/liquidity");
      }
    } else if (!!event.button) {
      // and if event.button, use 1 or 4 for middle button
  ...
  const focusInput = (event) => {
    if (
      !event.target.id.includes("balance_div") &&
      ...
      if (
        !isInFamilyTree(
          event.target,
          elem.getElementsByClassName(styles.choice_btn)[0]
        )
      ) {
  ...
  const toggleSettings = () => {
    ...
    if (settingsOn) {
      ...
      child = createElementFromHTML(
        `
            <div class="${styles.intrfc_settings_div_main}">
                <div class="${styles.intrfc_settings_title} ${
          styles.settings_title
        }">Interface Settings</div>
                <div class="${styles.exp_mode_div_main}">
                    <div class="${styles.exp_mode_div}">
                        <div class="${styles.info_row} ${
          styles.exp_mode_info_row
        }">
                            <div>
                                Expert Mode
                            </div>
                            <div class="${styles.question_icon} ${
          styles.tooltip
        }">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="#6e6e6e">
                                    <path d="M4.475 5.458c-.284 0-.514-.237-.47-.517C4.28 3.24 5.576 2 7.825 2c2.25 0 3.767 1.36 3.767 3.215 0 1.344-.665 2.288-1.79 2.973-1.1.659-1.414 1.118-1.414 2.01v.03a.5.5 0 0 1-.5.5h-.77a.5.5 0 0 1-.5-.495l-.003-.2c-.043-1.221.477-2.001 1.645-2.712 1.03-.632 1.397-1.135 1.397-2.028 0-.979-.758-1.698-1.926-1.698-1.009 0-1.71.529-1.938 1.402-.066.254-.278.461-.54.461h-.777zM7.496 14c.622 0 1.095-.474 1.095-1.09 0-.618-.473-1.092-1.095-1.092-.606 0-1.087.474-1.087 1.091S6.89 14 7.496 14z"/>
                                </svg>
                                <span class="${styles.tooltiptext_up} ${
          styles.tooltiptext_up_tip
        }">Allow high price impact trades and skip the confirm screen. Use at your own risk.</span>
                            </div>
                        </div>
                    </div>
                    <div class="${styles.switch_button}">
                        <input id="exp_input" class="${
                          styles.switch_button_checkbox
                        }" type="checkbox" ${
          expCheckboxOn ? "checked" : ""
        }></input>
                        <label class="${styles.switch_button_label}" htmlFor="">
                            <span class="${
                              styles.switch_button_label_span
                            }">Off</span>
                        </label>
                    </div>
                </div>
      ...
      child
        .getElementsByClassName(styles.switch_button_checkbox)[0]
        .addEventListener("click", (event) =>
          setExpCheckboxOn(event.target.checked)
        );
  ...
  const closeSettings = (event) => {
    if (
      !event.target.closest("#" + styles.settings_div) &&
      !isInFamilyTree(
        event.target,
        document.getElementById("settings_toggle_btn")
      )
    ) {
      if (
        document
          .getElementById(styles.settings_div)
          .classList.contains(styles.settings_div_activated)
      ) {
        setSettingsOn(false);
      }
    }
  ...
  const openExpModal = () => {
    ...
    html
      ...
      .addEventListener("click", () => setOpenedExpModal());
    html
  ...
  const openChooseModal = () => {
    ...
    const choosed_tokens =
      chooseTokenNum == 0 ? [token0, token1] : [token1, token0]; // [currently choosing token, another one]
    for (let popular_token of popularTokensCur) {
      popular_tokens_div.appendChild(
        getPopularTokenDiv(popular_token, choosed_tokens)
      );
    }
    ...
    if (!!tokenListCur.length) {
      for (let token of tokenListCur) {
        tokens_div.appendChild(
          getTokenDiv(token, choosed_tokens, is_overflown)
        );
      }
    } else {
  ...
  const handleSearchTokenInput = (chooseModalInput) => {
    const tokens_div = document.getElementsByClassName(
      styles.choose_modal_div_lower
    )[0];
    clearDiv(tokens_div);
    const choosed_tokens =
    ...
    if (chooseModalInput.match(regex)) {
      ...
      if (chooseModalInput == "") {
        ...
        for (let token of tokenListCur) {
          if (
            token["symbol"].toLowerCase().includes(chooseModalInputToLower) ||
  ...
  useEffect(() => {
    ...
    setIsBlockIcon(localStorage.getItem("isBlockIcon"));
  }, []);
